  ...
  it should "write and read files in different formats" in {
    withSparkSession { spark =>
      ...
      forAll(table) { formatStr =>
        ...
        format.write(originDF, path.toAbsolutePath.toString, WriteOperation.Append, Seq.empty[String])
        intercept[IllegalArgumentException](format.write(originDF, path.toAbsolutePath.toString, WriteOperation.Update, Seq.empty[String]))
